...
def main():
	...
	with open(metadataOriginal) as metadata:
		...
		with open(caminhoCSV, 'a') as csvFeatures:
			...
			# PRA CADA AUDIO DO METADATA.CSV
			cont = 0
			for audio in objReadCSV:
				...
				yOriginal, sr = librosa.load(arquivo, sr=int(freqAmostragem)*1000)

				# CADA AUDIO SERA DIVIDIDO DE FORMA A GERAR VARIOS DADOS NO CSV
				matrizFramesSeparados = librosa.util.frame(yOriginal, frame_length=FRAME_LENGTH, hop_length=OVERLAP_LENGTH).transpose()

				# PARA CADA FRAME EU CALCULO AS FEATURES SEPARADAMENTE
				for y in matrizFramesSeparados:
					...
					mfccMatriz = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13, n_fft=tamanhoY)
					...
					arrayFeatures.append(mediaRMS(y, tamanhoY))
					arrayFeatures.append(centroideEspectral(y, sr, tamanhoY))
					arrayFeatures.append(larguraEspectral(y, sr, tamanhoY))
					arrayFeatures.append(planicidadeEspectral(y, tamanhoY))
					arrayFeatures.append(rolloff(y, sr, tamanhoY))
					arrayFeatures.append(cruzamentosZero(y, tamanhoY))
